...
describe('dictionaries', () => {
  it('exposes Rwanda’s four working languages, all LTR', () => {
    assert.deepEqual([...LOCALES], ['rw', 'en', 'fr', 'sw']);
    assert.equal(LOCALE_META.length, 4);
    assert.ok(LOCALE_META.every((m) => m.dir === 'ltr'), 'no RTL locale remains');
    assert.equal(LOCALE_META.find((m) => (m.code as string) === 'ar'), undefined, 'Arabic removed');
    assert.equal(LOCALE_META.find((m) => (m.code as string) === 'ha'), undefined, 'Hausa removed');
  });

  it('isLocale accepts only known locales', () => {
    assert.equal(isLocale('rw'), true);
    assert.equal(isLocale('ha'), false);
