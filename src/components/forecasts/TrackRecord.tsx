...
export function TrackRecord({ record }: { record: TrackRecordData }) {
  ...
  return (
    <section aria-labelledby="tr-h" className="bg-ink/[0.03] border border-ink/10 rounded-2xl p-4 sm:p-5">
      <h2 id="tr-h" className="flex items-center gap-2 text-ink text-[15px] font-bold mb-1">
        <Award size={16} className="text-brand-ink" aria-hidden="true" />
      ...
      <div className="grid sm:grid-cols-3 gap-3 mb-5">
        <div className="bg-ink/5 rounded-xl p-3 text-center">
          <p className="text-ink/45 text-xs mb-1">{t(s.forecasts.accuracy)}</p>
          <p className="text-ink text-2xl font-bold">{pct(record.accuracy)}</p>
        </div>
        <div className="bg-ink/5 rounded-xl p-3">
          <p className="text-ink/45 text-xs mb-2">{locale === 'rw' ? 'Ukuri ku gicuruzwa' : 'By commodity'}</p>
          <ul className="space-y-1 text-[13px]">
            {record.byCommodity.map((b) => (
              <li key={b.commodity} className="flex justify-between text-ink/75">
                <span>{locale === 'rw' ? COMMODITIES[b.commodity]?.nameKiny : COMMODITIES[b.commodity]?.nameEn}</span>
                <span className="font-medium">{pct(b.accuracy)} <span className="text-ink/35">({b.evaluated})</span></span>
              </li>
            ))}
            {record.byCommodity.length === 0 && <li className="text-ink/35">—</li>}
          </ul>
        </div>
        <div className="bg-ink/5 rounded-xl p-3">
          <p className="text-ink/45 text-xs mb-2">{locale === 'rw' ? 'Ukuri ku gihe' : 'By horizon'}</p>
          <ul className="space-y-1 text-[13px]">
            {record.byHorizon.map((b) => (
              <li key={b.horizon} className="flex justify-between text-ink/75">
                <span>{b.horizon === '7d' ? (locale === 'rw' ? 'Iminsi 7' : '7 days') : b.horizon === '14d' ? (locale === 'rw' ? 'Iminsi 14' : '14 days') : (locale === 'rw' ? 'Iminsi 30' : '30 days')}</span>
                <span className="font-medium">{pct(b.accuracy)} <span className="text-ink/35">({b.evaluated})</span></span>
              </li>
            ))}
            {record.byHorizon.length === 0 && <li className="text-ink/35">—</li>}
          </ul>
        </div>
      </div>

      <h3 className="text-ink/60 text-xs font-semibold uppercase tracking-widest mb-2">
      ...
      <div className="space-y-2">
        {record.calibration.map((c) => (
          <div key={c.bin} className="flex items-center gap-3 text-xs">
            <span className="text-ink/50 w-16 shrink-0">{c.bin}</span>
            ...
            <span className="text-ink/70 w-20 text-right shrink-0">
              {pct(c.observed)} <span className="text-ink/35">({c.count})</span>
            </span>
          </div>
        ))}
      </div>
      <p className="text-ink/35 text-[11px] mt-3">
        {locale === 'rw'
