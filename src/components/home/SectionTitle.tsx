...
export function SectionTitle({ entry }: { entry: LangEntry }) {
  const { t } = useLocale();
  return (
    <h2 className="border-s-4 border-brand ps-3 text-lg font-bold text-ink">
      {t(entry)}
    </h2>
  );
}
...
export function ViewAllLink({ href }: { href: string }) {
  ...
  return (
    <Link
      href={href}
      className="inline-flex shrink-0 items-center gap-1 text-[13px] font-semibold text-brand-ink hover:underline"
    >
      {t(s.home.viewAll)}
