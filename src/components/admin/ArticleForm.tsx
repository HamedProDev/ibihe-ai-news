...
const inputCls =
  'mt-1 w-full bg-white/5 border border-white/15 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#00c853]/60';
const labelCls = 'text-white/60 text-[13px]';
...
export function ArticleForm({ initial, method, url }: { initial?: Article | null; method: 'POST' | 'PUT'; url: string }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-3 bg-[#111] border border-white/10 rounded-2xl p-4 sm:p-6">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <label className="block">
          ...
          <select value={v.category} onChange={(e) => set('category', e.target.value)} className={`${inputCls} bg-[#111]`}>
            {CATEGORIES.map((c) => (
        ...
        <label className="block">
          ...
          <select value={v.status} onChange={(e) => set('status', e.target.value)} className={`${inputCls} bg-[#111]`}>
            {STATUSES.map((st) => (
      ...
      <div className="flex gap-2 pt-1">
        <button
          type="submit"
          disabled={busy}
          className="bg-[#00c853] text-black text-sm font-semibold rounded-xl px-5 py-2.5 hover:bg-[#00e65f] disabled:opacity-40"
        >
        ...
        <button
          ...
          className="bg-white/10 text-white text-sm rounded-xl px-5 py-2.5 hover:bg-white/15"
        >
          {t(s.common.cancel)}
        </button>
      </div>
