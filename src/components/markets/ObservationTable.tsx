          ...
          {observations.map((o) => (
            <tr key={o.id} className="border-t border-white/5 text-white/80">
              <td className="px-3 py-2.5 font-medium text-white">
                ...
                {o.isMock && <span className="ml-1.5 text-[10px] text-amber-300/70">demo</span>}
              </td>
              <td className="px-3 py-2.5 whitespace-nowrap">
                {o.price.toLocaleString('en-US')} RWF<span className="text-white/40">/{o.unit}</span>
              </td>
              <td className="px-3 py-2.5">{o.pricePerKg != null ? o.pricePerKg.toLocaleString('en-US') : '—'}</td>
              <td className="px-3 py-2.5">{o.market} <span className="text-white/40">({o.district})</span></td>
              <td className="px-3 py-2.5 whitespace-nowrap text-white/60">{o.observedAt.slice(0, 10)}</td>
              <td className="px-3 py-2.5 text-white/60">{o.source.name}</td>
            </tr>
          ))}
        </tbody>
      </table>
