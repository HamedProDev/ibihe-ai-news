...
function TrendIcon({ direction }: { direction: MarketTrend['direction'] }) {
  if (direction === 'up') return <TrendingUp size={13} className="text-ok" aria-hidden="true" />;
  if (direction === 'down') return <TrendingDown size={13} className="text-danger" aria-hidden="true" />;
  if (direction === 'flat') return <Minus size={13} className="text-ink/40" aria-hidden="true" />;
  return <HelpCircle size={13} className="text-ink/30" aria-hidden="true" />;
}
...
export function MarketTrendCard({ trend }: { trend: MarketTrend }) {
  ...
  const stroke =
    trend.direction === 'up' ? 'var(--color-ok)' : trend.direction === 'down' ? 'var(--color-danger)' : 'var(--color-brand-ink)';

  return (
    <Link
      href={`/isoko/${trend.commodity}`}
      className="block bg-surface border border-ink/10 rounded-2xl p-4 hover:border-brand/40 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-brand"
    >
      <div className="flex items-center justify-between mb-1">
        <h3 className="text-ink font-semibold text-[15px]">{name}</h3>
      ...
      <p className="text-ink text-lg font-bold mb-2">
        ...
        <span className="text-ink/40 text-xs font-normal"> /kg</span>
      </p>
      <Sparkline points={trend.series.slice(-30)} width={200} height={48} stroke={stroke} />
      {trend.dataMode === 'demo' && (
        <p className="text-warn/70 text-[11px] mt-2">demo</p>
      )}
    </Link>
  );
}
