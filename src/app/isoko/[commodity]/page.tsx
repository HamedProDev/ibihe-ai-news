...
export default async function CommodityPage({ params }: { params: Promise<{ commodity: string }> }) {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-6">
      <nav aria-label="Breadcrumb" className="text-[13px] text-white/45 mb-3">
        <Link href="/isoko" className="hover:text-[#00c853]">Isoko</Link>
        {' / '}
        <span className="text-white/70">{info.nameKiny}</span>
      </nav>

      <h1 className="text-white text-2xl font-bold">{info.nameKiny}</h1>
      <p className="text-white/50 text-sm mb-5">{info.nameEn}</p>

      <DemoBanner mode={market.dataMode} />

      {trend && trend.series.length > 1 && (
        <section aria-label="Umurongo w’ibiciro" className="bg-[#111] border border-white/10 rounded-2xl p-4 mb-6">
          <div className="flex items-baseline justify-between mb-2">
            <p className="text-white text-2xl font-bold">
              {trend.latestPricePerKg?.toLocaleString('en-US')} <span className="text-sm font-normal text-white/45">RWF/kg</span>
            </p>
            <p className={`text-sm font-semibold ${trend.direction === 'up' ? 'text-green-400' : trend.direction === 'down' ? 'text-red-400' : 'text-white/50'}`}>
              {trend.changePercent != null ? `${trend.changePercent > 0 ? '+' : ''}${trend.changePercent}%` : '—'}
              <span className="text-white/35 font-normal"> / {trend.windowDays}d</span>
            </p>
          </div>
      ...
      {pending.length > 0 && (
        <section aria-labelledby="comm-fc" className="mb-6">
          <h2 id="comm-fc" className="text-white text-[15px] font-bold mb-3">Ibiteganyijwe — Forecasts</h2>
      ...
      <section aria-labelledby="comm-obs">
        <h2 id="comm-obs" className="text-white text-[15px] font-bold mb-3">Ibipimo — Observations ({market.observations.length})</h2>
        {market.observations.length > 0 ? (
          <ObservationTable observations={market.observations} />
        ) : (
          <p className="text-white/50 text-sm">Nta bipimo bibonetse.</p>
        )}
      </section>
    </main>
  );
