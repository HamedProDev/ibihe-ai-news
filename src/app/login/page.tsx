...
function LoginForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(next && next.startsWith('/') ? next : json.data.user.role === 'admin' ? '/admin/articles' : '/');
      router.refresh();
    } catch {
  ...
  return (
    <main className="max-w-md mx-auto px-4 py-16">
      <div className="bg-[#111] border border-white/10 rounded-2xl p-6">
        <h1 className="text-white font-bold text-xl mb-1">{locale === 'rw' ? s.auth.login.rw : s.auth.login.en}</h1>
        <p className="text-white/50 text-[13px] mb-5">
          {locale === 'rw' ? s.auth.loginSubtitle.rw : s.auth.loginSubtitle.en}
        </p>
        <form onSubmit={submit} className="space-y-3">
          ...
          <label className="block">
            <span className="text-white/60 text-[13px]">{locale === 'rw' ? s.auth.password.rw : s.auth.password.en}</span>
            <input
              type="password"
              required
          ...
          >
            {busy ? '…' : locale === 'rw' ? s.auth.login.rw : s.auth.login.en}
          </button>
        </form>
        <p className="text-white/50 text-[13px] mt-4 text-center">
          {locale === 'rw' ? s.auth.noAccount.rw : s.auth.noAccount.en}{' '}
