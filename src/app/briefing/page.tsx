  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-6">
      <div className="mb-5 flex items-center gap-3">
        <span className="flex size-11 items-center justify-center rounded-2xl bg-brand/15 text-brand-ink">
          <Bot size={22} aria-hidden />
        </span>
        <div>
          <h1 className="text-xl font-bold text-ink">{t(s.sidebar.aiSummary)}</h1>
          <p className="text-[13px] text-ink/50">{t(s.sidebar.poweredByEditors)}</p>
        </div>
      </div>
            ...
            {briefing.bullets.map((b, i) => (
              <li key={b.articleId}>
                <Link
                  ...
                  className="group flex gap-3 rounded-2xl border border-ink/10 bg-surface p-4 hover:border-ink/25"
                >
                  <span
                    aria-hidden
                    className="text-2xl font-extrabold leading-none text-ink/10 group-hover:text-brand-ink/40"
                  >
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  <span className="min-w-0">
                    <span className="block text-[15px] leading-relaxed text-ink/85 group-hover:text-ink">
                      {locale === 'rw' ? b.textKiny : b.textEn}
                    </span>
                    <span className="mt-1.5 inline-flex items-center gap-1 text-[13px] font-semibold text-brand-ink">
                      {t(s.home.readStory)}
