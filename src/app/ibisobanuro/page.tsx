...
export default function ExplainersPage() {
  return (
    <main className="max-w-3xl mx-auto px-4 py-6">
      <h1 className="text-ink text-2xl font-bold mb-1">Ibisobanuro</h1>
      <p className="text-ink/50 text-sm mb-6">
        Uburyo Ibihe ikora — mu mucyo. What happened → why it matters → what the data says → what could happen next.
      </p>
      <div className="space-y-4">
        {SECTIONS.map((s) => (
          <section key={s.titleEn} className="bg-ink/[0.03] border border-ink/10 rounded-2xl p-4 sm:p-5">
            <h2 className="flex items-center gap-2 text-ink text-[15px] font-bold mb-2">
              <s.icon size={16} className="text-brand-ink" aria-hidden="true" />
              {s.titleKiny}
            </h2>
            <p className="text-ink/75 text-sm leading-relaxed mb-2">{s.bodyKiny}</p>
            <p className="text-ink/45 text-[13px] leading-relaxed">{s.bodyEn}</p>
          </section>
        ))}
      </div>
    </main>
